...
Locate the active workspace's transcripts before fanning out. Use `$PI_SESSION_FILE`, or `~/.pi/agent/sessions/--<cwd>--/`. Use only that path. Don't glob across other workspace directories under `~/.pi/agent/sessions/`. That crosses workspace boundaries and reads private chats from unrelated projects.
