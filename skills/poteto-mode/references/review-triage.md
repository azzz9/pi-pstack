# Review-automation triage

Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles review-automation comments. The goal is not to ignore review automation by default. The goal is to stop treating every comment as a required code change.

## Decision rubric

Classify each review-automation thread before acting:
...
- Skip when: The PR description, screenshots, design review, or nearby code makes the visual change explicit, and the automation comment is only restating that a shared visual default changed.
...
### Upstack or stack-local usage a review automation cannot see

- Confidence: candidate
- Skip when: A review automation flags an export, component, helper, or file as unused, and the active forge's PR list and diffs, upper-stack diffs, or PR context show it is used by a later PR in the stack.
...
- Skip when: The comment body or a later review-automation reply explicitly says the finding is withdrawn, compliant, or a false positive, and the agent can verify the relevant rule locally.
...
- Confidence: candidate
- Skip when: Practically never. When a diff replaces native browser behavior with a manual equivalent (native sticky → JS-positioned clones, native scroll targeting → forwarded wheel/touch events, paint-order occlusion → masks/clip-path), the automation's logic-bug findings against that code have been consistently legitimate.
...
- Source: one sticky-occlusion PR: six review-automation passes, roughly eighteen findings, every one fixed rather than dismissed.
...
- Skip when: Never skip the verification itself; it costs one command. When a PR
  ...
  a SKILL.md, snapshot of doc wording), and a review automation claims "the test no longer
...
- Source: one prose-pinning PR with eight review-automation passes; the claim was real on
